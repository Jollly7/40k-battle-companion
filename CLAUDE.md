...
**Status:** v1.12.1 complete. Defender table rows that share a generic group name but carry different weapons (e.g. two "Neophyte Hybrid w/ Heavy Weapon ×2" with Seismic cannon vs Mining laser) are now disambiguated at parse time by appending a parenthetical suffix. Fully data-driven; only the displayed `name` changes. Re-import rosters to pick up the new names.
