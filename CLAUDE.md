...
- **Invuln in the six-column stat blocks** (`BrowseStatBlock` in `UnitPopOut`, `StatRow` in `CombatOverlay`): both take **`sv` (base save) and `invuln` as separate props** — never a pre-joined `"2+ (4++)"` string. An 8-character value wraps mid-token in a one-sixth-width cell at `text-xl`/`text-lg` (renders as `2+ (4+` / `+)`). The invuln is instead a small amber second line inside the Sv cell, with an equal-height empty spacer in the other five cells so the primary numbers stay on one baseline; when `invuln` is `null` no sub-line renders in any cell, so units without one gain no height. The inline `${SV} (${invuln})` form is still correct in `UnitAccordion`'s collapsed row and `UnitPopOut`'s merged-leader row — those are inline flow with no fixed cell.
...
| v1.13.3 | **Invuln saves displayed in `X++` notation**: new `src/utils/formatInvuln.js` normalises the parser's two stored shapes (`"4+"` from the 11th Ed `InSv` characteristic / `"Invulnerable Save (4+)"` ability match, and `"4++"` from the bare-name fallback) to standard 40k `X++` notation on read. Applied at all five display sites — `UnitAccordion` collapsed row, `UnitPopOut` browse stat block and merged-leader stats row, `CombatOverlay` `AttackerCard` stat row and Defender table `InvSv` column. Parser unchanged, so rosters already in Cloudflare KV need no re-import. Riptide Battlesuit now reads `2+ (4++)` instead of `2+ (4+)`. **Stat-block overflow fix**: the extra character made the joined string wrap mid-token in the six-column grid blocks, so `BrowseStatBlock` and `StatRow` now take `sv` and `invuln` as separate props and render the invuln as an amber sub-line inside the Sv cell (empty equal-height spacer in the other cells keeps baselines level; no sub-line at all when there is no invuln). Inline sites unchanged. | ✅ Done |
